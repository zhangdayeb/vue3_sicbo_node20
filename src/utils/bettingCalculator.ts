...
export class BettingUtils {
   ...
   * 获取赔率显示文本（仅用于UI显示）
   */
  static getOddsDisplay(betType: BetType): string {
   ...
   * 计算潜在收益（仅供参考，真实计算在后端）
   */
  static calculatePotentialWin(betType: BetType, amount: number): {
    minWin: number
    maxWin: number
    display: string
  } {
   ...
   * 获取投注类型的颜色主题
   */
  static getBetTypeColor(betType: BetType): string {
   ...
   * 检查投注类型是否为高风险投注
   */
  static isHighRiskBet(betType: BetType): boolean {
    const highRiskBets = [
   ...
   * 获取投注建议文本
   */
  static getBetAdvice(betType: BetType): string {
